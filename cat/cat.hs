...
-- readFile simple takes a filename and reads it, returning an IO String
-- putStr takes in a string and returns IO ()

-- \a -> ... Is just a lambda function.

-- mapM makes more sense with it's signature, Monad m => (a -> m b) -> [a] -> m [b]
--      It takes in a function and a list, and applies the function to each element of the list
...
-- Tha '>=>' is composition of monads (Monad m => (a -> m b) -> (b -> m c) -> (a -> m c))
-- So here we compose (readFile :: FilePath -> IO String) and (putStr :: String -> IO() )
--      to get a function (FilePath -> IO) that takes in a file and prints it to standard out
